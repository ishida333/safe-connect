...
<<<<<<< HEAD
  try {
    ...
    // Fallback: generate realistic shelter names based on location
    return generateFallbackShelters(lat, lng);
  }
};

const generateFallbackShelters = (lat: number, lng: number): Shelter[] => {
  const buildingNames = [
    { name: '市立総合体育館', type: '指定避難所' },
    { name: '中央公民館', type: '指定避難所' },
    { name: '第一小学校', type: '指定避難所' },
    { name: '市民文化会館', type: '広域避難場所' },
    { name: '総合運動公園', type: '広域避難場所' },
    { name: '区民センター', type: '指定避難所' },
    { name: '市立図書館', type: '指定避難所' },
    { name: '福祉センター', type: '指定避難所' },
  ];

  return buildingNames.slice(0, 5).map((b, i) => ({
    id: i + 1,
    name: b.name,
